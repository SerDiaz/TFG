...
void Estructura::asignar(int ta, int nav, int npi, QString fe, QString *noV, QString *ori, QString *des, QString *pi, float *ini, float *fin, QString *avi,int taAr, Estructura *est)
{
...
//        Creando aviones
        aviones->addSpacing(23);
        QLabel *tamano = new QLabel(tr("Avion 1"));
...
//    LE DAMOS COLOR A LA ESCENA(UN GRIS IGUAL QUE EL RESTO DE LA VENTANA
    ...
    scene->setSceneRect(0,0,1275,392);
...
//                        ESTO ME AYUDARÁ A SABER CUAL ES LA RUTA DE CADA PILOTO PARA PINTARLO DESPUÉS
    ...
    for(int i=0;i<tam;i++){
        ...
        if(ult==0){
            ...
            yRectangle1=(30+20.5* avionSiguiente[posicion])-(largo/2)+10.25;
            yRectangle2=(30+20.5*( avionSiguiente[posicionSiguiente]))-(largo/2)+10.25;
...
//                        SI ESTÁN MUY JUNTOS LOS VUELOS NO SOBREPONER LOS NOMBRES. SOLO SE PINTA EL FINAL
                    ...
                    if(vCercano==0){
                        ...
                        ori->setPos(xRectangle-25,yRectangle-2);
                    ...
                    des->setPos(posDes,yRectangle-2);
                    des->setPlainText(destino[i]);
                    scene->addItem(des);


                   rectangulo[i].setScene(scene);
                   rectangulo[i].setPosArray(i);
                   rectangulo[i].posandscale(xRectangle,yRectangle,tamRectangle,20.5);
                   rectangulo[i].name_posFligth(lista[k],auxNomPosicion);
                   scene->addItem(rectanInd[i]);
